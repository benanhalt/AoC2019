...
data Portal = Portal {key :: String, loc :: Pos, dir :: PortalDir} deriving Show
data PortalDir = Inner | Outer deriving Show
...
main = do
  ...
  let s0 = State {pos = loc entrance, steps = 0}
...
rememberState :: State -> S.Set Pos -> S.Set Pos
rememberState State {pos} seen = S.insert pos seen

search :: Grid -> [Portal] -> Pos -> Queue State -> S.Set Pos -> Int
search grid portals exit q seen =
  let
    s@State {pos, steps} = front q

    newStates = do
      let Pos (x,y) = pos
      pos' <- [(x-1, y), (x, y-1), (x+1, y), (x, y+1)]
      guard $ not $ S.member (Pos pos') seen
      let s' = State {pos = Pos pos', steps=steps+1}
      guard $ isGood grid s'
      pure s'
    ...
    newStates' = newStates <> do
      ...
      guard $ not $ S.member (loc dest) seen
      traceShowM dest
      pure $ State {pos = loc dest, steps=steps+1}

    q' = foldl' (flip snoc) (rest q) newStates'
    seen' = foldl' (flip rememberState) seen newStates'

  in
    if pos == exit then steps
