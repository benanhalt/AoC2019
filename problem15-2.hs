...
main = do
  ...
  print $ length $ unfoldr diffuseO2 (room droid)

diffuseO2 :: Room -> Maybe (Room, Room)
diffuseO2 room =
  if length vacuumEdge < 1 then Nothing else Just (room', room')
  where
    room' :: Room = foldr (\p -> Map.insert p OxygenSys) room vacuumEdge
    vacuumEdge = filter isEdge $ fmap fst $ filter ((== Empty) . snd) $ Map.toList room
    isEdge p = any id $ do
      d <- [North, South, East, West]
      let n = updatePosition d Moved p
      pure $ Map.lookup n room == Just OxygenSys
