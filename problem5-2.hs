...
main :: IO ()
main = do
  -- forM_ running $ \s -> putStrLn $ show $ s
  putStrLn $ show $ stateF
  putStrLn $ show $ reverse $ outputs stateF
  where
    (running, notRunning) = break halted $ iterate step state0
